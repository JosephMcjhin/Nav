...
FString UNavigationMathLibrary::GetRelativeDirectionText(
  ...
  if (AngleDegrees >= 165.0f || AngleDegrees < -165.0f) {
    return UTF8_TO_TCHAR(u8"向后转");
  }
  if (AngleDegrees > 0.0f) {
    return FString::Printf(TEXT("%s%d%s"), UTF8_TO_TCHAR(u8"向右转 "),
                           FMath::RoundToInt(AngleDegrees),
                           UTF8_TO_TCHAR(u8" 度"));
  }
  return FString::Printf(TEXT("%s%d%s"), UTF8_TO_TCHAR(u8"向左转 "),
                         FMath::RoundToInt(-AngleDegrees),
                         UTF8_TO_TCHAR(u8" 度"));
}
